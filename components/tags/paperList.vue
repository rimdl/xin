...
onMounted(()=>{
  if (props.tagName){
    const skip = 0;
    get_json(baseUrl+"/api/papers/getpapersbytag?tag="+props.tagName+"&limit="+config.page_size+"&sort=desc&skip="+skip,{}).then(res=>{
      papers.value = res
    })
  }
  else {
...
const getPapersByTag = (tagName:string)=>{
  const skip = 0;
  get_json(baseUrl+"/api/papers/getpapersbytag?tag="+tagName+"&limit="+config.page_size+"&sort=desc&skip="+skip,{}).then(res=>{
    papers.value = res
    systemStore.setNotification({
      msg:"共查找到"+res.length+"篇文章",
      type:""
    })
  })
}

watch(() => props.tagName, (newVal) => {
  if (newVal){
